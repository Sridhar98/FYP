...
def draw_labels_and_boxes(img, boxes, confidences, classids, idxs, colors, labels):
    if len(idxs) > 0:
        for i in idxs.flatten():
            ...
            f = open(output_path+str(FLAGS.image_path.split('.')[0])+"_yolo3.txt", "a")
...
def modified_YOLO(x,y,w,h):
        f = open(output_path+str(FLAGS.image_path.split('.')[0])+"_erroryolo.txt", "a")
        f.write(str(x*random.uniform(0.85, 1.1))+"\t"+str(y*round(random.uniform(0.85, 1.1)))+"\t"+str((x+w)*round(random.uniform(0.85, 1.1)))+"\t"+str((y+h)*round(random.uniform(0.85, 1.1)))+"\n")
        f.close()
...
if __name__ == '__main__':
	...
	Files_List = [str(FLAGS.image_path.split('.')[0])+"_yolo3.txt", str(FLAGS.image_path.split('.')[0])+"_erroryolo.txt", str(FLAGS.image_path.split('.')[0])+"_ssd.txt", str(FLAGS.image_path.split('.')[0])+"_gt.txt"]
